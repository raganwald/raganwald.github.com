---
title: "Transducers, Generators, and Composition"
layout: default
...
In the second post, [What's a Transducer?], we looked at the [transducer] pattern, a technique that the JavaScript community has adopted from Clojure.[^CLU] With transducers, we decomposed the `reduce`/`reduceWith` function such that we could work with a reducer function and composeable transformations of the reducer function.

Composing transformations of the reducer function permits us to write separate, single responsibility steps for a transformation, again without incurring the cost of building intermediate copies of the data set.

[What's a Transducer?]: http://raganwald.com/2017/04/30/transducers.html
[transducer]: https://clojure.org/reference/transducers
[^CLU]: JavaScript adopted transducers from Clojure, just as JavaScript also adopted generators from Python, which adopted generators from other languages in a chain stretching back to Barbara Liskov's CLU, the language that popularized coroutines.

In this post we will compare the iterators and generators approach to the transducer pattern, and embellish on the very basic implementation we used in [What's a Transducer?]

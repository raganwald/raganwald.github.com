...
From this, it follows that if the largest left truncatable prime we've seen is of the form x..., if we find ourselves testing whether some number that is greater than 9x... is left truncatable, we know that there are no larger left truncatable primes.
...
In fact, we don't need to save *every* truncatable prime, just those that are one digit smaller than whatever prime we're currently examining. Likewise, we don't need to do a lot of fancy math to figure out if a prime we're checking is too big or not: If it is more than one digit longer than the longest truncatable prime in our collection, we know that it can't be tractable, and that there are a finite number of truncatable primes.

...
If you physically babysist this algorithm while it runs, you'll see that it gets slower and slower as it goes. We needn't detail how to instrument our code, but if we count how many primes it has to check to discover each truncatable prime, we find that although it gyrates back and forth a lot, the number of primes it tests grows rapidly as it finds longer and longer truncatable primes.
...
And it's worse than this. Generating the consecutive primes is itself a process that gets slower and slower as each prime is generated. And we need to generate more and more primes to disciver each truncatable prime.

But at least we made it work. Can we make it faster?

...
Well, it is going to be on the order of the size of our input. It's possible that every time we evaluate an expression like `x => !x`, we get a new function object. It would be nice if our implementation knew enough to hoist it out of our function for us and make it a constant, but even if it did, when we evaluate `compose(accFn, x => !x)`, we are *absolutely* creating a new function object.

So one way or the other, we are going to end up with a lot of function objects.

...
Unlike `isEven`, the work to be done between the recursive call and the return statement is not fixed, so for our work function, we will need a closure. Let's begin with an anonymous function:

...
const _isEven =
  (myself, n) =>
    (console.log(n), (n === 0) || !myself(n - 1));

const isEven = why(_isEven);

isEven(1000)
  //=> Go for coffee, then return to find out that the answer is `true`

isEven(1001)
  //=> Go for another coffee, then return to find out that the answer is `false`
```

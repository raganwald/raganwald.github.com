...
And here's Sharleen's solution, building upon a [widowbird](/2018/08/30/to-grok-a-mockingbird.html), a recursive combinator that implements trampolining:
...
1. Every Balanced Parentheses string is either empty or contains at least one empty pair “`()`”;
2. Removing any Balanced Parentheses substring from a Balanced Parentheses string leaves a Balanced Parentheses string as the remainder, and;
3. Therefore any Balanced Parentheses string is either empty, or contains an empty pair that can be removed, leaving a shorter Balanced Parentheses string. Repeat until empty or the attempt to remove an empty pair fails.

Proof of points one and two can be worked out from first principles or from existing literature on Dyck Languages.

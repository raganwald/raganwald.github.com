...
Modelling the real world is a tool for exploring ideas in software and for helping others read our software: What is familiar, subjectively feels "intuitive."[^raskin] But when the modelling interferes with our ability to understand our softeare's behaviour, we should relax our desire to make everything about modelling. What we seek is maximum understandability and maximum flexibility, not maximum fidelity.
...
function sow(initialState, fromPit, skipPit = fromPit) {
  ...
  return [pits, pitToSow, pits[pitToSow]];
}
```
...
```javascript
let gamePits = Array(12).fill(4);
let lastPit, seedsInLastPit;

[gamePits, lastPit, seedsInLastPit] = sow(gamePits, 0);
...
The lesson learned is that decoupling functions from mutable state makes them easier to understand and test. And if we want to model the "real world" by having a value representing the current game state, we can do that anyways:
...
[gamePits, lastPit, seedsInLastPit] = sow(gamePits, lastPit, initialChoice);
[gamePits, lastPit, seedsInLastPit] = sow(gamePits, lastPit, initialChoice);
[gamePits, lastPit, seedsInLastPit] = sow(gamePits, lastPit, initialChoice);
[gamePits, lastPit, seedsInLastPit] = sow(gamePits, lastPit, initialChoice);

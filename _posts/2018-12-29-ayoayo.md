...
If we draw the call "tree" for `fib`, it's not linear, it's a tree with two branches per node. That's why it's bi-recursive. We won't get into them here, but there's also recursion with more than two or a variable number of invocations of itself. Such functions are considered _multiply recursive_.

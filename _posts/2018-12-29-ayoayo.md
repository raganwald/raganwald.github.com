...
[^Dependency-Injection]: James Sinclair calls this [Dependency Injection](https://jrsinclair.com/articles/2018/how-to-deal-with-dirty-side-effects-in-your-pure-functional-javascript/).
...
### implementing the third rule, with a digression into encapsualtion
...
In our original version, it didn't, and thus it is _possible_ for the environment to optimize the original version by hoisting `atLeastOne` outside of `atLeastOnePossibleMove`, in effect rewriting our code in "flat functions" form behind the scenes. But when we take advantage of scope, the environment cannot rewrite our code. Does that matter?[^hoisting]
...
If performance is not a bottleneck, prefer the code that is easiest to read. But let's move on and get started on `permissibleFilter`:

...
For large numbers, `isEven` invokes itself, and that invocation invokes itself, and *that* invocation invokes itself, and so forth all the way down to `0` or `1` like turtles. But although at one time there may be many invocations of `isEven` at once, each one has only invoked another invocation of `isEven` exactly once.

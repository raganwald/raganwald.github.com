...
- We take all of the references to shared state that functions must read, and replace them with parameters.
- We take all of the references to shared state that functions must write, and replace them with creating *copies* of the shared state. We then return those copies along with any other return values the functions already have.
...
Well. At first sight, there seem to be more moving parts with this approach. But in truth, they were always there, it's just that we "lifted" them into the interface of the `scoop`, `distribute`, and `sow` functions where we can see them. And now, we can test any arbitrary invocation without having to remember to set things up in advance.

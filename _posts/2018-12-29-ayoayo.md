...
In programming, a _recursive function_ is a function that invokes itself directly or indirectly. A *linearly recursive function* is a function where each invocation that invokes itself (directly or indirectly) at most once before returning.

For example, this linearly recursive function computes whether a non-negative integer is even or not:
...
For large numbers, `isEven` invokes itself, and that invocation invokes itself, and *that* invocation invokes itself, and so forth all the way down to the base (a/k/a "terminal") conditions `0` or `1`, like turtles. But although at one time there may be many invocations of `isEven` at once, each one has only invoked another invocation of `isEven` exactly once.

If we draw the call "tree" for `isEven`, it's actually a vertical line. That's why it's linearly recursive: Its recursion is a straight line from input to base or terminal case:

<div class="mermaid">
  graph TD
    six["isEven(6)"] --> four["isEven(4)"]
    four             --> two["isEven(2)"]
    two              --> zero["isEven(0)"]
</div>

Not all recursive functions are linear. Here's a naïve version of [fibonacci] that is _bi-recursive_: Each invocation invokes itself two times:
...
If we draw the call "tree" for `fib`, it's not linear, it's a tree with two branches per node. That's why it's bi-recursive.

<div class="mermaid">
  graph TD
    six["fib(6)"] --> six5["fib(5)"]
    six           --> six4["fib(4)"]
    ...
    six543        --> six5432["fib(2)"]
    six543        --> six5431["fib(1)"]
    six542        --> six5421["fib(1)"]
    six542        --> six5420["fib(0)"]
    six532        --> six5321["fib(1)"]
    six532        --> six5320["fib(0)"]
    six432        --> six4321["fib(1)"]
    six432        --> six4320["fib(0)"]

    six5432       --> six54321["fib(1)"]
    six5432       --> six54320["fib(0)"]
</div>

We won't get into it here, but there's also recursion where a function invokes itself more than two, or a variable number of times. Such functions are _multiply recursive_. However, all of the recursive functions we've written here are linearly recursive.
...
function doSomethingOrOther (collection) {
  ...
  for (const element of collection) {
    ...
    // frobish the collection element by element
  }

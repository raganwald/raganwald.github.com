...
If we draw the call "tree" for `fib`, it's not linear, it's a tree with two branches per node. That's why it's bi-recursive. We won't get into it here, but there's also recursion where a function invokes itself more than two, or a variable number of times. Such functions are _multiply recursive_.

All of the recursive functions we've written here are linearly recursive.

...
The first thing we need is a function to scoop up the seeds from a pit:

```javascript
function scoop(fromPit) {
  const seedsInHand = gamePits[fromPit];
...
We'll also need a function to distribute seeds. It needs to know which pit to skip, where to start distributing, and how many seeds to are in the hand that need to be distributed:

```javascript
function distribute(skipPit, currentPit, seedsInHand) {
...
The first thing our `distrubute` function needs to do is place a seed from the hand into the current pit. With a wrinkle that if the current pit is the skip pit, move to the next pit:
...
If we still have seeds, then what? We need to keep sowing. We *could* rewrite `distribute` to have a loop of so kind, maybe `do { ... } while (...)`. But hang on!

`distribute` is a function that takes a pit to skip, a current pit, and a number of seeds in hand. It returns the result of sowing seeds. That's what we need to return, with just one alteration: We need distribute starting with the next pit. So let's just do that:
...
function distribute(skipPit, currentPit, seedsInHand) {
  ...
  if (seedsInHand === 0) {
    ...
    return distribute(skipPit, nextPit(currentPit), seedsInHand);
  }
}
```
...
Having a single variable, `gamePits`, describing the state of the game in progress does feel intuitively sound. There is only one board in the game, and when we sow seeds, we change it. So why shouldn't we "model the real world accurately?"
...
Relay sowing ends when the last seed is placed in an empty hole. In Ayoayo, if that empty hole is on the player's side, they capture all of the seeds in the corresponding hole on the opponent's side:

```javascript
function capturedSeeds(before, startPit, endPit) {
  const endedOnThePlayersSide = startPit < 6 === endPit < 6;

  if (endedOnThePlayersSide) {
    const pitOnOpponentsSide = 11 - endPit;
    const after = before.slice(0);
    const seeds = after[pitOnOpponentsSide];
...
const [afterTurn, captured] = capturedSeeds(afterSowing, startPit, endPit);
...
If we want to update the game state, we'll need a notion of a "score," and we can change `capturedSeeds` to actually handle the update:

```javascript
function capturedSeeds(beforePits, beforeScore, startPit, endPit) {
  const endedOnThePlayersSide = startPit < 6 === endPit < 6;
...
const [afterSowing, endPit] = relaySow(gameStart, startPit);
const [afterTurn, scoreAfter] = capturedSeeds(afterSowing, scoreStart, startPit, endPit);

...
The other kind of sowing, as used in other games from the same family, is just called _sowing_. The sowing stops when the last seed is sown, regardless of whether the last seed lands in an occupied or unoccupied pit.

It's fairly obvious that if we make a function for sowing, we can use that to make a function for relay sowing, so let's start with an ordinary sowing function. We'll make thew relay sowing function later.

Now our function will start with a pit where started our turn (which we need to know so that we can avoid sowing into it), the current pit we're on before moving to the next pit (which defaults to the start pit), and some notion of the number of seeds we're holding (which defaults to zero if we don't say otherwise).

We'll start by scooping up the seeds if we're on the start pit.

```javascript
function sow(startPit, currentPit = startPit, seedsInHand = 0) {

  // empty the pit into the hand
  if (currentPit === startPit) {
    seedsInHand = gamePits[startPit];
    gamePits[startPit] = 0;
  }
...
Ok, if we have seeds in hand after emptying the start pit into the hand (or after being supplied with a different value), then we have to move to the next pit. If it's the start pit, we have to move to the next pit *again*. Then we sow a seed:
...
function sow(startPit, currentPit = startPit, seedsInHand = 0) {
  ...
  // empty the pit into the hand
  if (seedsInHand === 0) {
    seedsInHand = gamePits[currentPit];
    gamePits[currentPit] = 0;
  }

  currentPit = ++currentPit % gamePits.length;
...
After all this, we will have moved to the next pit and sown one seed. What should we do next?

Well, if we don't have any more seeds in hand, we're done. We should return the current pit so that other code can do things like work out whether to continue sowing, or determine whether to capture any seeds from our opponent:
...
If we still have seeds, then what? We need to keep sowing. We *could* rewrite `sow` to have a loop of so kind, maybe `do { ... } while (...)`. But hang on! If `sow` is a function that can presumably return the result of sowing seeds if we tell it that we have seeds in hand (which it is), we can actually say that if we sow a seed and still have seeds in hand, the result to return is that ow sowing the remaining seeds, like this:
...
function sow(startPit, currentPit = startPit, seedsInHand = 0) {
  ...
  if (seedsInHand === 0) {
    ...
    return sow(startPit, currentPit, seedsInHand);
  }
}
```
...
Now, there were five seeds in the last pit (pit `4`). If there are two or more seeds in the last pit, it wasn't empty before we sowed the last seed into it. So when relay sowing, we'd sow again, only this time we'll tell our function start at pit `4`, where we left off last time:
...
We're off to a reasonable start. And as it happens, the recursive solution is actually straightforward to explain and to implement. Naturally, there are excess copies of arrays involved and so forth, and there are ways of working around that if we like. But this is good enough, so let's move on.
...
```javascript
function sow(before, startPit, currentPit = startPit, seedsInHand = 0) {
  let after = before.slice(0);

  // empty the pit into the hand
  if (seedsInHand === 0) {
    seedsInHand = after[currentPit];
    after[currentPit] = 0;
  }

  currentPit = ++currentPit % after.length;
...
```javascript
function relaySow(before, startPit, currentPit = startPit) {
  let [after, lastPit] = sow(before, startPit, currentPit);
...
We *could* rewrite `relaySow` to have a loop of so kind, maybe `do { ... } while (...)`. But hang on! With `sow`, we recursively called `sow` when there was more work to be done. Let's use the same pattern, the code writes itself:
...
function relaySow(before, startPit, currentPit = startPit) {
  ...
  if (after[lastPit] === 1) {
    ...
    return relaySow(after, startPit, lastPit);
  }
}

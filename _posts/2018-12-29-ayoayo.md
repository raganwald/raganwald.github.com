...
function possibleFilter(pits, moves) {
  return moves.filter( pit => pits[pit] > 0 );
}
```
...
const possibleMovesForPlayerOne = possibleFilter(afterTurn, potentialMovesForPlayerOne);

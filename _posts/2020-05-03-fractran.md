...
Could we "flatten" the registers by encoding their value into a single register? Could we develop a Minsky Machine that only needs one tape?
...
The final part of our Single-Register Minsky Machine will be its input and output. With a Magnificent or Marvellous Minsky Machine, we listed the initial values of the registers. So for adding having a value of `0` in register 1, `3` in register 2, and `13` in register 3, we would include the parameters `0, 3, 13`.
...
If you are extremely keen, try writing a program that simulates Single-Register Minsky Machines. We want to write something like:
...
It works. And that's not all...

*Tomorrow, the final chapter will reveal all. But I'm sure you can work out what comes next on your own!*

...
*This prelude is long on nostalgia, and short on programming. If that does not interest you, skip straight to the* [Table of Contents](#table-of-contents).
...
- [a magnificent minsky machine](#a magnificent minsky machine)
- [operating the magnificent minsky machine](#operating-the-magnificent-minsky-machine)
- [a notation for magnificent minsky machine](#a-notation-for-magnificent-minsky-machine)
...
- [n j wildberger's lecture on the collatz conjecture](#n-j-wildbergers-lecture-on-the-collatz-conjecture)
- [notes](#notes)
...
Let's write a program using a Magnificent Minsky Machine. Now that we have the general idea of how these machines operate, here is how we will specific each Magnificent Minsky Machine, i.e. How we shall write its "program:"
...
### operating the magnificent minsky machine
...
Consider a Magnificent Minsky Machine that will add two numbers. We will only need one state for this, with two rules.
...
Our notation for rules is very simple. In each state, there is a comma-separated list of rules. Our machine only has one state, `1`, and it has two rules: `(1^1)/(2^1)` and `(1^1)/(3^1)`. Newlines aren't significant, they're whitespace like anything else. So we could also write our program like this, if we feel that's easier to digest:
...
Each rule has an action clause of one or more tuples of the form `(t^s)`, where `t` identifies the tape-head, and `s` identifies the number of squares to move forward, followed by a `/`, followed by a guard clause of one or more tuples also of the form `(t^s)`, where `t` again specifies the tape-head, and `s` specifies the number of squares to move back towards the beginning if possible.
...
Now we'll write a Magnificient Minsky Machine that multiplies two numbers. We will use more than one state, so we separate states with `;` and show a transition to a different state with `→`.
...
N.J. Wildberger has an excellent [video introduction][cc] to the Collatz Conjecture.
...
With what we've learned so far, FRACTRAN appears to be nothing more than a clever Gödel encoding for a class of register machines. It is amusing to write programs in fractions instead of with states and registers, but if there is nothing more to it, FRACTRAN doesn't present us with any new information about computability that we didn't already know from researching Minsky Machines.
...
### n j wildberger's lecture on the collatz conjecture

...
Our notation for rules is very simple. In each state, there is a comma-separated list of rules. Our machine only has one sate, `One`, and it has two rules: `2-1, 1+1 -> One` and `3-1, 1+1 -> One`. Each rule has zero or more actions of the form `t+s`, where `t`t identifies the tapehead, and `s` identifies the number of squares to move forward, followed by zero or more guards of the form `t-s`, where `t` again specifies the tapehead, and `s` specifies the number of squares to move back towards the beginning if possible.

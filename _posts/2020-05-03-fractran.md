...
### a marvellous minsky multiplication machine
...
The diagram doesn't do it justice without understanding the order of the rules within each state, so let's move to a text format. Elaborating on our notation somewhat by introducing the notion of separating states with `;`, our marvellous minsky multiplication machine can be written as:

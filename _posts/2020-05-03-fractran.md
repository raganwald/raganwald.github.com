...
This can, of course, be automated. If you're keen, have a go at it before reading this example. If you're **really** keen, see if you can devise a different algorithm for deriving a Marvellous Minsky Machine from any Magnificent Minsky Machine.[^eminem]

[^eminem]: The suggested algorithm for automatically deriving a Marvellous Minsky Machine from any Magnificent Minsky Machine actually only works for the subset of Magnificent Minsky Machines that do not use a transfer to state 0 as an explicitly halt.<br/><br/>A good exercise for the enthusiastic reader is to answer either or both of the following questions: _For every Magnificent Minsky Machine that explicitly halts, is there an equivalent Magnificent Minsky Machine that only halts implicitly? If so, how can we derive it?_
...
With what we've learned so far, FRACTRAN might appear to be little more than applying Gödel encoding to a class of register machines. It is amusing to write programs in fractions instead of with states and registers, but if there was nothing more to it, FRACTRAN wouldn't present us with any new information about computability that we didn't already know from researching Minsky Machines.
...
(Discuss on [Hacker News](https://news.ycombinator.com/item?id=23142232) and [/rprogramminglanguages](https://www.reddit.com/r/ProgrammingLanguages/comments/ghpek3/remembering_john_conways_fractran_a_ridiculous/))

...
<center><img src=“/assets/images/fractran/Collatz5nplus1simulator.png”></center>

*[Collatz 5N+1 simulator][c5nplus1] is an unknown fate pattern constructed by David Bell in December 2017 that simulates the Collatz 5N+1 algorithm using sliding block memory and p1 technology, while always having a population below 32000.*[^collatz-life]
...
Consider writing the Collatz conjecture like this:

- Start with n.
- With every iteration, consider two rules:
  1. If n is divisible by two: Divide n by 2, multiply the result by 1, and start again.
  2. If rule 1 does not apply, but n is divisible by 1: Divide n by 1, multiply the result by 3, add 1, and start again.

Now consider a related set of functions, those that:

- Iterate over a natural number, _n_.
- Have one or more rules, not necessarily two.
- Each rule is guarded by whether n can be divided by some natural number, and if so, divides n by that number and then multiplies the result by another number.
- The rules are consulted in sequence. The first rule to apply is used and the iteration starts again.

This set of functions is like the 3n+1 function, but omits addition or subtraction. These functions are FRACTRAN programs: The rules of the function are the fractions of the program. 

Conway called these _Collatz functions_. And a question as to whether every, or no value of _n_ reaches a particular integer (such as 1), he called an _arbitrary Collatz problem_.

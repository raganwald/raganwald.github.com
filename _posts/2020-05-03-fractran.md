...
Wiith prime factorization, we encode lists of numbers as exponents of consecutive primes. Thus, the list `[1, 9, 6, 2]` is encoded as 2¹3⁹5⁶7², or 30,139,593,750. While the numbers might become very large, they're workable for the kinds of toy exercises we're performing here.

Prime factorization can obviously be used to encode the Masterful Machine's state, using the exponents of consecutive primes as virtual registers. But that's not its only use. Action clauses in our rules are tuples of a register and an amount to increment. So `(3^2)` means, "increment the register identified as `3`, by 2."
...
And that, of course, evaluates to a fraction like `182/55`.

If you are extremely keen, try writing a program that simulates Masterful Minsky Machines.

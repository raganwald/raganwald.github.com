...
As Wikipedia notes, FRACTRAN is a Turing-complete esoteric programming language invented by John Conway. A FRACTRAN program is an ordered list of positive fractions together with an initial positive integer input *n*. The program is run by updating the integer *n* as follows:

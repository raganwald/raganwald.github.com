...
[^even-more-marvellous]: Even more marvellously, for every Magnificent Minsky Machine with two or more tapes, there also exists an equivalent "Master" Minksy Machine with just one tape, and more states. But we are getting ahead of ourselves.
...
![Godeleinstein](/assets/images/conway/godeleinstein.jpg)
...
[Numbering]: https://en.wikipedia.org/wiki/Gödel_numbering
...
In a "Magnificent" Minsky Machine, the current state of the world is respresented by the machine's current state and the current state of its registers. But a "Marvellous" Minksky Machine does away with the machine's current state. the "state of the world" is encoded entirely by the contents of its registers.

This brings up an interesting possibility: We were able to "flatten" a Magnificent Minsky Machine into a Marvellous Minsky Machine, getting rid of its states by encoding the machine states into registter values.

Could we "flatten" the registers by encoding their value into a single register? Could we develop a "Masterful" Machine that doesn't need more than one tape?

### the masterful machine
...
Gödel chose to use [prime factorization][factorization], and that method suits us perfectly.

[factorization]: https://en.wikipedia.org/wiki/Integer_factorization "Integer Factorization"

Wiith prime factorization, we encode lists of numbers as exponents of consecutive primes. Thus, the list `[1, 9, 6, 2]` is encoded as `2^1 * 3^9 * 5^6 * 7^2`, or 30,139,593,750. To decode 30,139,593,750, we divide by 2, 3, 5, 7, and so forth. While the numbers might become very large, they're workable for the kinds of toy exercises we're performing here.

Prime factorization can obviously be used to encode the Masterful Machine's state, using the exponents of consecutive primes as virtual registers. But that's not its only use. Action clauses in our rules are tuples of a register and an amount to increment. So `(3,2)` means, "increment the register identified as `3` by two."
...
Now we see the obvious (no doubt you saw it much more quickly than I did when I first read about these subjects). We can represent action and guard clauses as natural numbers with prime factorization. So a rule like `(1,1)(4,1)(6,1)/(3,1)(5,1)` would become `182/55`.

...
In a "Magnificent" Minsky Machine, the current state of the world is represented by the machine's current state and the current state of its registers. But a "Marvellous" Minksky Machine does away with the machine's current state. the "state of the world" is encoded entirely by the contents of its registers.
...
In the paper, they present a universal machine, `U`, with 37 rules. It can simulate a Minsky Machine, just as a Universal Turing Machine can simulate a Turing Machine.

What does this tell us about FRACTRAN?
...
2. Therefore, there exists a FRACTRAN program, POLYGAME, that can simulate any FRACTRAN program.

And here it is, John Horton Conway's POLYGAME:

```
583/559, 629/551, 437/527, 82/517, 615/329, 371/129,
  1/115,   53/86,   43/53,  23/47,  341/46,   41/43,
  47/41,   29/37,   37/31, 299/29,   47/23,  161/15,
 527/19,   159/7,    1/17,   1/13,    1/3
```

There is a wonderful explanation of POLYGAME's workings in [Open Problems in Communication & Computation](/assets/fractran/open-problems-in-communication-and-computation-1987.pdf). Now that we've explored the relationship between register machines and FRACTRAN, you're more than equipped to read his original words explaining his original creation.

And there can be no greater tribute to Conway, than to absorb his ideas as he chose to convey them.

(Discuss  on [proggit](https://www.reddit.com/r/programming/comments/gfx6r4/untitled_fractran_game/))

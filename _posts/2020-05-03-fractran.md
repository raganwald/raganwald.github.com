...
The Collatz game consists of generating the Collatz sequence for _n_, and stopping when when its next term is the target value, when its next term is undefined, or when the sequence enters a loop.[^infinitegame]

[^infinitegame]: It is possible for a Collatz game to run without halting. A trivial example is the 1-partite Collatz function 2n. If started with any value n | n > 1, it will never include 1, and also never loop.

...
Dyck languages are not restricted to parentheses. For example, we can use "1" and "0" instead of "(" and ")," and while the language with 1s and 0s will have a different vocabulary, it will be exactly isomorphic to the language with (s and )s. *is est*, `'10'`, `'1100', `'1010'`, and `'1101010010'` are valid, while `'1110'`, `'1000'`, and `'01'` are not.

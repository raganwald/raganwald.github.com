...
Dyck languages are not restricted to literal parentheses. The symbols we use comprise a language's alphabet. Our simplest example of balanced parentheses uses `(` and `)`, so its alphabet is `()`. And although we will talk about languages having "balanced parentheses," the symbols can be anything we choose, even things we don't usually call parenthesesm, like `↗` and `↙` or `«` and `»`.

For example, we can use `1` and `0` instead of `(` and `)`, and while the language with 1s and 0s will have a different vocabulary, it will be exactly isomorphic to the language with (s and )s. `10`, `1100`, `1010`, and `1101010010` are words in the language that , while `1110`, `1000`, and `01` are not. The possible parentheses in a language comprise its alphabet. Languages with one pair of parentheses are called Dyck-1 languages. 

The language with (s and )s and the language with 1s and 0s are called Dyck-1 languages, because they have what we will call one pair of parentheses. Can a Dyck language have more than one pair of parentheses? Of course! Dyck languages with more than one pair are called *typed* Dyck languages, because the type of parentheses must match as well as the open and closed parentheses must match. And of course, Dyck-1 languages are *untyped*, because the only matching to be done is open versus closed.

Here are some Dyck languages:

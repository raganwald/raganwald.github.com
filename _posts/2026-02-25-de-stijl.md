...
Dyck languages don't need to contain the symbls we call parentheses and use in mathematics and programming. For example, we can use "1" and "0" instead of "(" and ")," and while the language with 1s and 0s will have a different vocabulary, it will be exactly isomorphic to the language with (s and )s. *is est*, `'10'`, `'1100', `'1010'`, and `'1101010010'` are valid, while `'1110'`, `'1000'`, and `'01'` are not.

The language with (s and )s and the language with 1s and 0s are called Dyck-1 languages, because they have what we will call one pair of parentheses. Can a Dyck language have more than one pair of parentheses? Of course! Dyck languages with more than one pair are called *typed* Dyck languages, because the type of parentheses must match for the string to be balanced. And of course, Dyck-1 languages are *untyped*:

Here are some example languages:

|Pairs|Designation    |Alphabet          |Example      |
|----:|:--------------|:-----------------|:------------|
|1    |Dyck-1, Untyped|()                |((()()))(()) |
|1    |Dyck-1, Untyped|10                |101010111000 |
|3    |Dyck-3, Typed  |(), [], {}        |{()[]}{}     |
|5    |Dyck-5, Typed  |«», ⸘‽, <>, ¿?, ¡!|⸘<>‽«»       |

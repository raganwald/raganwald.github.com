...
![Architype Van Doesburg, created by Theo Van Doesburg, 1919](/assets/de-stijl/architype-van-doesburg.png)
...
1. Dyck labguages can be recognized with both [pushdown automata]][A Brutal Look at Balanced Parentheses, Computing Machines, and Pushdown Automata] and [recursive regular expressions][A Brutal Look at Balanced Parentheses, Computing Machines, and Pushdown Automata].
1. Dyck languages are *concatenative*:
  a. If `A₁` and `A₂` are words in the Dyck language with vocabulary `Vᴬ`, then the word `A₁A₂` is also a word in the Dyck language with alphabet `Vᴬ`.
  b. ᴰ ᴱ If `D` is a word in the Dyck language with alphabet `Vᴰ`, and `E` is a word in the Dyck language with alphabet `Vᴱ`, then the word`DE` will be a word in the Dyck language with alphabet `Vᴰ | Vᴱ`, the *union* of the two alphabets.
1. There are several ways to express the grammar of Dyck languages, but we will use `S → (S)S` for Dyck-1 languages, and `S → (S)S | [S]S | {S}S` for Dyck-3 languages.

...
1. Dyck languages are *concatenative*:
  - If `A₁` and `A₂` are words in the Dyck language with vocabulary `Vᴬ`, then the word `A₁A₂` is also a word in the Dyck language with alphabet `Vᴬ`.
  - ᴰ ᴱ If `D` is a word in the Dyck language with alphabet `Vᴰ`, and `E` is a word in the Dyck language with alphabet `Vᴱ`, then the word`DE` will be a word in the Dyck language with alphabet `Vᴰ | Vᴱ`, the *union* of the two alphabets.

(let* (
	(nav-date (dictionary-ref (first (file->entity "NAVajo/funds.tbn")) "navpsdate"))
	(compound-date (dictionary-ref (first (file->entity "NAVajo/funds.tbn")) "returndate"))
	(moneymarket (first (file->entity "NAVajo/mm.tbn")))
	(clean	(lambda (value) (if (== value "n/a") "--" value)))
	(make-rows (lambda (fundType fundCategory)
		(let* (
			(detect-eqv
				(macro	(aCollection aLabel aValue)
				`(detect ,aCollection (lambda (anElement) (eqv? (dictionary-ref anElement ,aLabel) ,aValue)))))
			(write-select
				(macro	(aCollection aSelectFunc aWriteFunc)
				`(inject-into
					,aCollection
					"" ; seed
					(lambda (seed element)
						(if (,aSelectFunc element)
							(+
								seed
								(,aWriteFunc element))
							seed))))))
			(write-select
				(file->entity "NAVajo/funds.tbn") ; collection to write-select over
				(lambda (fund) ; selection function
					(let* (
							...
							(clean-fund-number
								(if (null? raw-fund-number)
									""
									raw-fund-number))
							(lookup (detect-eqv (file->entity "NAVajo/NAVajoLookup.tbn") "FUNDNUMBER" clean-fund-number)))
							(and
								(false? (null? lookup))
								(== (dictionary-ref lookup "CATEGORY") fundCategory)
								(== (dictionary-ref lookup "TYPE") fundType))))
				(lambda	(fund) ; write function
						(let* (
							(raw-fund-number (dictionary-ref fund "fundnumber"))
							(clean-fund-number
								(if (null? raw-fund-number)
